...
## Business Case:
  ...
  Bellabeat wants to gain a greater foothold in the wearable fitness tracker
marketplace by understanding how women use competing products. The company is interested in usage trends for competing products. They would like to apply the findings from this analysis to one of their existing products to drive sales and increase customer base.
...
# convert ActivityDate column into date object
activity_fixed <- activity %>% mutate(ActivityDate = mdy(activity$ActivityDate))
sleep_fixed <- sleep %>% mutate(SleepDay = mdy_hms(sleep$SleepDay))
# check for full duplicates
sum(duplicated(activity_fixed))
sum(duplicated(sleep_fixed)) # returns 3 duplicated rows
# identify and remove duplicates
filter(sleep_fixed, duplicated(sleep_fixed))
sleep_fixed <- distinct(sleep_fixed)
sum(duplicated(sleep_fixed))
```

## Analysis Summary
  Summary statistics for the activity_fixed table. Approximately 8.19% of entries contain a total of 0 steps, suggesting that the device was worn for the given day. Curiously, 0.21% (or 7 rows) of the observations record the daily maximum of sedentary minutes while also having tracked steps. The discrepancy could perhaps be do to the way the device calculates the two metrics.
  
```{r Analysis}
# general table stats
n_distinct(activity_fixed$Id)
row_count <- nrow(activity_fixed)
...
zero_steps_count <- count(activity_fixed %>%
  filter(TotalSteps == 0))
# count of rows that record max amount of sedentary minutes
sedentary_max <- count(activity_fixed %>%
...
```{r}
ggplot(data=activity_fixed, aes(x=TotalSteps, y=SedentaryMinutes, color=Id)) + geom_point() 
```
...
```{r}
ggplot(data=sleep_fixed, aes(x=TotalMinutesAsleep, y=TotalTimeInBed, color=Id)) + geom_point()
```
...
combined_data <- merge(sleep_fixed, activity_fixed, by="Id", all=TRUE)
...
mean_very_active <- mean(activity_fixed$VeryActiveMinutes)
mean_fairly_active <- mean(activity_fixed$FairlyActiveMinutes)
mean_lightly_active <- mean(activity_fixed$LightlyActiveMinutes)
...
# FIXIT: needs to group dates by week before plotting
ggplot(weight_table, aes(x = Date, y = WeightPounds, group = Id, cut_width(Date, 7))) +
  geom_line() +
  geom_point()
```
